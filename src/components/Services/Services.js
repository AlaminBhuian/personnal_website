import React, {Component, Fragment} from 'react';
import {Col, Container, Row} from 'react-bootstrap';
import "../../asset/css/bootstrap.min.css"
import "../../asset/css/custom.css"
import graphicsLogo from "../../asset/image/graphicsLogo.svg"
import mobileLogo from "../../asset/image/mobileLogo.svg"
import webLogo from "../../asset/image/webLogo.svg"

class Services extends Component {
    render() {
        return (
            <Fragment>
                <Container className="text-center">
                    <h1 className="serviceTitle">SERVICES</h1>
                    <Row>
                        <Col lg={4} md={6} sm={12}>
                            <div className="serviceCard text-center" >
                                <img src={webLogo}></img>
                                <h4 className="serviceName">Web Development</h4>
                                <p className="serviceDescription">I design and develop static and dynamic web sites as per your requirements as we believe, “web is world’s next home”.</p>
                            </div>
                        </Col>
                        <Col lg={4} md={6} sm={12}>
                            <div className="serviceCard text-center" >
                                <img src={mobileLogo}></img>
                                <h4 className="serviceName">Mobile Development</h4>
                                <p className="serviceDescription">I build native and cross platfrom mobile app for your business and instiution as per as your requirements.</p>
                            </div>
                        </Col>
                        <Col  lg={4} md={6} sm={12}>
                            <div className="serviceCard text-center" >
                                <img src={graphicsLogo}></img>
                                <h4 className="serviceName">Graphics Design</h4>
                                <p className="serviceDescription">I desing modern user interface and other graphical components for your business and instiution.</p>
                            </div>
                        </Col>
                    </Row>

                </Container>

            </Fragment>
        );
    }
}

export default Services;